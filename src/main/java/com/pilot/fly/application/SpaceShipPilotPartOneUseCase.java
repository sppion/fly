...
public class SpaceShipPilotPartOneUseCase implements CommandLineRunner {
    ...
    public SpaceShip moveSpaceShip(SpaceShip spaceShip, Direction action, int amount){
        switch (action) {
            case FORWARD -> spaceShip.setHorizontalPosition(spaceShip.getHorizontalPosition()+amount);
            case UP -> spaceShip.setAltitude(spaceShip.getAltitude()-amount);
            case DOWN -> spaceShip.setAltitude(spaceShip.getAltitude()+amount);
            default -> {
            }
        }
        return spaceShip;
